...
namespace ThaleTheGreat.CosmeticRingsRedux.Framework.Critters
{
    internal class DustSprite : NPC
    {
        ...
        public DustSprite(Vector2 position) : base(new AnimatedSprite("Characters\\Monsters\\Dust Spirit"), position * 64f, 2, "DustSprite", null)
        {
            base.Breather = false;
        ...
        {
            b.Draw(this.Sprite.Texture, base.getLocalPosition(Game1.viewport) + new Vector2(32 + ((base.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), 64 + base.yJumpOffset), this.Sprite.SourceRect, Color.White, base.rotation, new Vector2(8f, 16f), new Vector2(base.Scale + (float)Math.Max(-0.1, (double)(base.yJumpOffset + 32) / 128.0), base.Scale - Math.Max(-0.1f, (float)base.yJumpOffset / 256f)) * 4f, base.flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, base.drawOnTop ? 0.991f : ((float)base.StandingPixel.Y / 10000f)));
            if (base.isGlowing)
            {
                b.Draw(this.Sprite.Texture, base.getLocalPosition(Game1.viewport) + new Vector2(32f, 64 + base.yJumpOffset), this.Sprite.SourceRect, base.glowingColor * base.glowingTransparency, base.rotation, new Vector2(8f, 16f), Math.Max(0.2f, base.Scale) * 4f, base.flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, base.drawOnTop ? 0.99f : ((float)base.StandingPixel.Y / 10000f + 0.001f)));
            }
            b.Draw(Game1.shadowTexture, base.getLocalPosition(Game1.viewport) + new Vector2(32f, 80f), Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 2f + (float)base.yJumpOffset / 64f, SpriteEffects.None, (float)(base.StandingPixel.Y - 1) / 10000f);
        }
