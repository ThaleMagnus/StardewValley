...
internal sealed class ThreadedTaskScheduler : TaskScheduler, IDisposable {
    ...
    private static int DefaultConcurrencyLevel => Math.Max(1, Environment.ProcessorCount - 1);
    ...
    private void TryCancelTask(Task task) {
        try {
            if (TryExecuteTask(task) || task.IsCompleted) {
                task.Dispose();
            }
            ...
            Debug.Warning("Failed to complete disabled threaded task", ex);
        }
    }
