...
internal sealed class AnimalHusbandryModule : WalletModule
{
    ...
    private void PatchCoreGameMethods()
    {
        ...
        MethodInfo? doesItemExistAnywhere = AccessTools.Method(typeof(Utility), nameof(Utility.doesItemExistAnywhere), new[] { typeof(string) });
        MethodInfo? doesItemExistAnywherePostfix = AccessTools.Method(typeof(DoesItemExistAnywherePatch), nameof(DoesItemExistAnywherePatch.Postfix));
        if (doesItemExistAnywhere is not null && doesItemExistAnywherePostfix is not null)
            Harmony.Patch(doesItemExistAnywhere, postfix: new HarmonyMethod(doesItemExistAnywherePostfix));
    }
        ...
        e.Edit(asset =>
        {
            ...
            powers[WalletPowerId] = new PowersData
            {
                DisplayName = state.GetDisplayName(
                    Monitor,
                    Host.Translate("item.meat-wand.name"),
                    Host.Translate("item.meat-cleaver.name")
                ),
    ...
    private void RegisterGmcm()
    {
        ...
        try
        {
            ...
            AddBool(gmcm, nameof(Config.ModEnabled), () => Config.ModEnabled, SetModEnabled, "config.animal-husbandry.enabled.name", "config.animal-husbandry.enabled.tooltip");
            AddBool(gmcm, nameof(Config.AutoUseEnabled), () => Config.AutoUseEnabled, value => SetAutoUseMasterEnabled(value, false), "config.animal-husbandry.auto-use.name", "config.animal-husbandry.auto-use.tooltip");
            AddBool(gmcm, nameof(Config.RequireLeftControlForAutoUse), () => Config.RequireLeftControlForAutoUse, value => Config.RequireLeftControlForAutoUse = value, "config.animal-husbandry.require-control.name", "config.animal-husbandry.require-control.tooltip");
            AddBool(gmcm, nameof(Config.PlayToolSwapSound), () => Config.PlayToolSwapSound, value => Config.PlayToolSwapSound = value, "config.animal-husbandry.swap-sound.name", "config.animal-husbandry.swap-sound.tooltip");
            AddBool(gmcm, nameof(Config.ShowHudMessageWhenStored), () => Config.ShowHudMessageWhenStored, value => Config.ShowHudMessageWhenStored = value, "config.animal-husbandry.stored-message.name", "config.animal-husbandry.stored-message.tooltip");
            AddKeybind(gmcm, nameof(Config.ToggleAutoUseHotkey), () => Config.ToggleAutoUseHotkey, value => Config.ToggleAutoUseHotkey = value, "config.animal-husbandry.toggle-auto-use.name", "config.animal-husbandry.toggle-auto-use.tooltip");
            AddKeybind(gmcm, nameof(Config.UseMeatToolHotkey), () => Config.UseMeatToolHotkey, value => Config.UseMeatToolHotkey = value, "config.animal-husbandry.use-tool.name", "config.animal-husbandry.use-tool.tooltip");
            GmcmRegistered = true;
        }
        catch (Exception ex)
        {
    ...
    {
        gmcm.AddBoolOption(ModManifest, getValue, setValue, () => Host.Translate(nameKey), () => Host.Translate(tooltipKey), fieldId);
    }

    private void AddKeybind(IGenericModConfigMenuApi gmcm, string fieldId, Func<StardewModdingAPI.Utilities.KeybindList> getValue, Action<StardewModdingAPI.Utilities.KeybindList> setValue, string nameKey, string tooltipKey)
    {
        gmcm.AddKeybindList(ModManifest, getValue, setValue, () => Host.Translate(nameKey), () => Host.Translate(tooltipKey), fieldId);
    }
    ...
    private void SetAutoUseMasterEnabled(bool enabled, bool showMessage)
    {
        ...
        if (Context.IsWorldReady)
        {
            ...
            if (showMessage)
                Game1.addHUDMessage(new HUDMessage(Host.Translate(enabled ? "hud.animal-husbandry-auto-use-enabled" : "hud.animal-husbandry-auto-use-disabled"), HUDMessage.newQuest_type));
        }
    ...
    private void ConvertInventoryTools(Farmer player)
    {
        ...
        if (Config.ShowHudMessageWhenStored)
            Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.animal-husbandry-tool-stored"), HUDMessage.newQuest_type));
    }
    ...
    private void DisableWalletBehavior(Farmer player, bool showMessage)
    {
        ...
        if (showMessage && returnedTool)
            Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.animal-husbandry-tool-returned"), HUDMessage.newQuest_type));
    }
    ...
    private bool ShouldReportStoredMeatTool(string itemId)
    {
        return Context.IsWorldReady
            && IsMeatToolEnabled()
            && GetStoredTool(Game1.player) is not null
            && (string.Equals(itemId, MeatToolItemId, StringComparison.OrdinalIgnoreCase)
                || string.Equals(itemId, MeatToolQualifiedItemId, StringComparison.OrdinalIgnoreCase));
    }
    ...
    private static class DoesItemExistAnywherePatch
    {
        ...
        {
            if (!__result && Instance?.ShouldReportStoredMeatTool(__0) == true)
                __result = true;
        }
    }
    ...
    private sealed class MeatToolState
    {
        ...
        public string GetDisplayName(IMonitor monitor, string meatWandFallback, string meatCleaverFallback)
        {
            ...
            {
                MeatWandMenuSpriteIndex => meatWandFallback,
                MeatCleaverMenuSpriteIndex => meatCleaverFallback,
                _ when tool is not null && !string.IsNullOrWhiteSpace(tool.DisplayName) => tool.DisplayName,
                _ => meatCleaverFallback
            };
        }
