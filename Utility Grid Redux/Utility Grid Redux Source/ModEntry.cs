...
public sealed class ModEntry : Mod
{
    ...
    private void OnAssetRequested(object? sender, AssetRequestedEventArgs e)
    {
        if (e.NameWithoutLocale.IsEquivalentTo("Data/BigCraftables"))
        {
            e.Edit(asset =>
            {
                ...
                AddBigCraftable(data, "BronzeWaterPump", T("machine.bronze-water-pump.name"), RuleDescription(Config.EnableBronzeWaterPumpRule, ProducedDescription(Config.BronzeWaterPumpWaterProduced, "water") + " " + ConsumedDescription(Config.BronzeWaterPumpPowerConsumed, "power")), 100, pumpTexture, 0);
                AddBigCraftable(data, "SteelWaterPump", T("machine.steel-water-pump.name"), RuleDescription(Config.EnableSteelWaterPumpRule, ProducedDescription(Config.SteelWaterPumpWaterProduced, "water") + " " + ConsumedDescription(Config.SteelWaterPumpPowerConsumed, "power")), 1000, pumpTexture, 1);
                AddBigCraftable(data, "GoldWaterPump", T("machine.gold-water-pump.name"), RuleDescription(Config.EnableGoldWaterPumpRule, ProducedDescription(Config.GoldWaterPumpWaterProduced, "water") + " " + ConsumedDescription(Config.GoldWaterPumpPowerConsumed, "power")), 10000, pumpTexture, 2);
                AddBigCraftable(data, "IridiumWaterPump", T("machine.iridium-water-pump.name"), RuleDescription(Config.EnableIridiumWaterPumpRule, ProducedDescription(Config.IridiumWaterPumpWaterProduced, "water") + " " + ConsumedDescription(Config.IridiumWaterPumpPowerConsumed, "power")), 100000, pumpTexture, 3);
                AddBigCraftable(data, "UtilityGridBattery", T("machine.utility-grid-battery.name"), RuleDescription(Config.EnableUtilityGridBatteryRule, StorageDescription(50, "power", Config.UtilityGridBatteryPowerConsumed, Config.UtilityGridBatteryPowerProduced)), 1000, storageTexture, 0);
                AddBigCraftable(data, "UtilityGridWaterTank", T("machine.utility-grid-water-tank.name"), RuleDescription(Config.EnableUtilityGridWaterTankRule, StorageDescription(50, "water", Config.UtilityGridWaterTankWaterConsumed, Config.UtilityGridWaterTankWaterProduced)), 1000, storageTexture, 1);
                AddBigCraftable(data, "UtilityGridAdvancedBattery", T("machine.utility-grid-advanced-battery.name"), RuleDescription(Config.EnableUtilityGridAdvancedBatteryRule, StorageDescription(100, "power", Config.UtilityGridAdvancedBatteryPowerConsumed, Config.UtilityGridAdvancedBatteryPowerProduced)), 5000, storageTexture, 2);
    ...
    {
        return enabled ? description : T("description.rule-disabled");
    }

    private static string ProducedDescription(int amount, string resource)
    {
        return T("description.produces", new { amount = NormalizeProducedAmount(amount), resource = ResourceName(resource) });
    }

    private static string ConsumedDescription(int amount, string resource)
    {
        return T("description.consumes", new { amount = NormalizeConsumedAmount(amount), resource = ResourceName(resource) });
    }

    private static string StorageDescription(int capacity, string resource, int consumed, int produced)
    {
        return T("description.storage", new { capacity, resource = ResourceName(resource), consumed = NormalizeConsumedAmount(consumed), produced = NormalizeProducedAmount(produced) });
    }
    ...
    private bool QueuePipeAction(string locationName, Vector2 tile, GridKind grid, int index, int rotation, PipeActionKind action)
    {
        ...
        SetGridEditStatus(T("grid.status.line-failed", new { action = T(action == PipeActionKind.Place ? "grid.action.place" : "grid.action.delete"), x = (int)tile.X, y = (int)tile.Y }));
        return false;
    }
    ...
    private void OnModMessageReceived(object? sender, ModMessageReceivedEventArgs e)
    {
        ...
        if (e.Type == nameof(PipeActionRequest))
        {
            ...
            if (TryApplyPipeAction(request, farmer, chargePlayer: true, playFeedback: false, out PipeActionBroadcast broadcast))
                ...
                Helper.Multiplayer.SendMessage(new PipeActionRejection { TranslationKey = "pipe-action-rejected" }, nameof(PipeActionRejection), new[] { ModId }, new[] { e.FromPlayerID });
            return;
        }
        ...
        if (e.Type == nameof(PipeActionRejection))
        {
            ...
            if (!string.IsNullOrWhiteSpace(rejection.TranslationKey))
                Game1.showRedMessage(T(rejection.TranslationKey));
        }
    }
    ...
    {
        string text = T("grid.edit-hud", new { place = Config.PlaceTile, delete = Config.DestroyTile, switchGrid = Config.SwitchGrid, shape = Config.SwitchTile, rotate = Config.RotateTile });
        int padX = 16;
        int padY = 10;
    ...
    private void RegisterBetterCrafting()
    {
        ...
        betterCrafting.CreateDefaultCategory(false, categoryId, () => T("better-crafting.category"), recipes, "Bronze Water Pump", false, null);
    ...
    private void RegisterGmcm()
    {
        ...
        gmcm.AddSectionTitle(ModManifest, () => T("config.section.general"));
        gmcm.AddBoolOption(ModManifest, () => Config.EnableMod, value => Config.EnableMod = value, () => T("config.enable-mod.name"));
        gmcm.AddBoolOption(ModManifest, () => Config.EnablePowerRules, value => Config.EnablePowerRules = value, () => T("config.enable-power-rules.name"));
        gmcm.AddBoolOption(ModManifest, () => Config.EnablePipeIrrigation, value => Config.EnablePipeIrrigation = value, () => T("config.enable-pipe-irrigation.name"), () => T("config.enable-pipe-irrigation.tooltip"));
        gmcm.AddBoolOption(ModManifest, () => Config.WaterSurroundingTiles, value => Config.WaterSurroundingTiles = value, () => T("config.water-surrounding-tiles.name"));
        gmcm.AddBoolOption(ModManifest, () => Config.ShowWateredTileMarkers, value => Config.ShowWateredTileMarkers = value, () => T("config.show-watered-markers.name"));
        gmcm.AddBoolOption(ModManifest, () => Config.ShowSprinklerAnimations, value => Config.ShowSprinklerAnimations = value, () => T("config.show-sprinkler-animations.name"));
        gmcm.AddBoolOption(ModManifest, () => Config.DebugLogging, value => Config.DebugLogging = value, () => T("config.debug-logging.name"));
        gmcm.AddNumberOption(ModManifest, () => Config.PercentWaterPerTile, value => Config.PercentWaterPerTile = NormalizeWaterPerTile(value), () => T("config.water-per-tile.name"), () => T("config.water-per-tile.tooltip"), 0f, 1f, 0.05f);

        gmcm.AddSectionTitle(ModManifest, () => T("config.section.pipe-costs"));
        gmcm.AddNumberOption(ModManifest, () => Config.PipeCostGold, value => Config.PipeCostGold = value, () => T("config.pipe-gold-cost.name"), null, 0, 100000, 1);
        gmcm.AddNumberOption(ModManifest, () => Config.PipeDestroyGold, value => Config.PipeDestroyGold = value, () => T("config.pipe-gold-refund.name"), null, 0, 100000, 1);
        gmcm.AddTextOption(ModManifest, () => Config.PipeCostItems, value => Config.PipeCostItems = value, () => T("config.pipe-item-cost.name"), () => T("config.pipe-items.tooltip"));
        gmcm.AddTextOption(ModManifest, () => Config.PipeDestroyItems, value => Config.PipeDestroyItems = value, () => T("config.pipe-item-refund.name"), () => T("config.pipe-items.tooltip"));
        gmcm.AddTextOption(ModManifest, () => Config.PipeSound, value => Config.PipeSound = value, () => T("config.pipe-create-sound.name"));
        gmcm.AddTextOption(ModManifest, () => Config.DestroySound, value => Config.DestroySound = value, () => T("config.pipe-destroy-sound.name"));

        gmcm.AddSectionTitle(ModManifest, () => T("config.section.controls"));
        gmcm.AddKeybindList(ModManifest, () => Config.ToggleGridOverlay, value => Config.ToggleGridOverlay = value, () => T("config.toggle-overlay.name"), () => T("config.toggle-overlay.tooltip"));
        gmcm.AddKeybindList(ModManifest, () => Config.ToggleGrid, value => Config.ToggleGrid = value, () => T("config.toggle-editing.name"), () => T("config.toggle-editing.tooltip"));
        gmcm.AddKeybindList(ModManifest, () => Config.SwitchGrid, value => Config.SwitchGrid = value, () => T("config.switch-grid.name"));
        gmcm.AddKeybindList(ModManifest, () => Config.SwitchTile, value => Config.SwitchTile = value, () => T("config.change-shape.name"));
        gmcm.AddKeybindList(ModManifest, () => Config.RotateTile, value => Config.RotateTile = value, () => T("config.rotate-shape.name"));
        gmcm.AddKeybindList(ModManifest, () => Config.PlaceTile, value => Config.PlaceTile = value, () => T("config.place-pipe.name"));
        gmcm.AddKeybindList(ModManifest, () => Config.DestroyTile, value => Config.DestroyTile = value, () => T("config.destroy-pipe.name"));

        gmcm.AddSectionTitle(ModManifest, () => T("config.section.machine-rules"), () => T("config.section.machine-rules.tooltip"));
    ...
    private void AddContentPackRuleOptions(IGenericModConfigMenuApi gmcm)
    {
        ...
        gmcm.AddSectionTitle(ModManifest, () => T("config.section.content-pack-rules"), () => T("config.section.content-pack-rules.tooltip"));

        string? previousSource = null;
        foreach (LoadedContentPackRule loaded in ContentPackRules.OrderBy(rule => rule.SourceName, StringComparer.OrdinalIgnoreCase).ThenBy(GetContentPackRuleDisplayName, StringComparer.OrdinalIgnoreCase))
        {
            ...
            AddMachineRuleToggle(gmcm, () => GetContentPackRuleConfig(loaded).Enabled, value => GetContentPackRuleConfig(loaded).Enabled = value, loaded.Rule.DisplayName, () => RuleDescription(GetContentPackRuleConfig(loaded).Enabled, BuildContentPackRuleDescription(loaded)), loaded.Rule.Keys, () => GetContentPackRuleDisplayName(loaded));
    ...
    private void AddContentPackProducedOption(IGenericModConfigMenuApi gmcm, LoadedContentPackRule loaded, string resourceName, Func<int> getValue, Action<int> setValue)
    {
        gmcm.AddTextOption(
            ...
            () => T("config.resource.produced.name", new { resource = ResourceName(resourceName) }),
            () => T("config.content-pack-resource.produced.tooltip", new { resource = ResourceName(resourceName), machine = GetContentPackRuleDisplayName(loaded), min = MinProducedAmount, max = MaxProducedAmount }),
            null,
            null,
    ...
    private void AddContentPackConsumedOption(IGenericModConfigMenuApi gmcm, LoadedContentPackRule loaded, string resourceName, Func<int> getValue, Action<int> setValue)
    {
        gmcm.AddTextOption(
            ...
            () => T("config.resource.consumed.name", new { resource = ResourceName(resourceName) }),
            () => T("config.content-pack-resource.consumed.tooltip", new { resource = ResourceName(resourceName), machine = GetContentPackRuleDisplayName(loaded), min = MinConsumedAmount, max = MaxConsumedAmount }),
            null,
            null,
    ...
    private void AddMachineRuleToggle(IGenericModConfigMenuApi gmcm, Func<bool> getValue, Action<bool> setValue, string machineName, Func<string>? tooltip = null, IEnumerable<string>? itemLookupKeys = null, Func<string>? displayName = null)
    {
        gmcm.AddBoolOption(
            ModManifest,
            getValue,
            setValue,
            () => displayName?.Invoke() ?? GetMachineDisplayName(machineName),
    ...
    private static string GetGmcmMachineTooltip(string machineName, bool enabled, string? utilityDescription, IEnumerable<string>? itemLookupKeys = null)
    {
        ...
        utilityDescription = string.Equals(utilityDescription, T("description.rule-disabled"), StringComparison.OrdinalIgnoreCase)
            ? string.Empty
            : utilityDescription?.Trim();

        if (!enabled)
            return !string.IsNullOrWhiteSpace(baseDescription) ? baseDescription : T("description.rule-disabled");

        if (string.IsNullOrWhiteSpace(baseDescription))
            return string.IsNullOrWhiteSpace(utilityDescription) ? T("config.machine.disable-tooltip") : utilityDescription;
    ...
    private void AddProducedOption(IGenericModConfigMenuApi gmcm, Func<int> getValue, Action<int> setValue, string resourceName)
    {
        gmcm.AddTextOption(
            ...
            value => setValue(ParseProducedAmountText(value, getValue())),
            () => T("config.resource.produced.name", new { resource = ResourceName(resourceName) }),
            () => T("config.resource.produced.tooltip", new { resource = ResourceName(resourceName), min = MinProducedAmount, max = MaxProducedAmount })
        );
    }
    ...
    private void AddConsumedOption(IGenericModConfigMenuApi gmcm, Func<int> getValue, Action<int> setValue, string resourceName)
    {
        gmcm.AddTextOption(
            ...
            value => setValue(ParseConsumedAmountText(value, getValue())),
            () => T("config.resource.consumed.name", new { resource = ResourceName(resourceName) }),
            () => T("config.resource.consumed.tooltip", new { resource = ResourceName(resourceName), min = MinConsumedAmount, max = MaxConsumedAmount })
        );
    }
    ...
    {
        return SHelper.Translation.Get(key).ToString();
    }

    private static string T(string key, object tokens)
    {
        return SHelper.Translation.Get(key, tokens).ToString();
    }

    private static string ResourceName(string resource)
    {
        return T("resource." + resource.Trim().ToLowerInvariant());
    }

    private static string GetMachineDisplayName(string machineName)
    {
        string slug = Regex.Replace(machineName.Trim().ToLowerInvariant(), "[^a-z0-9]+", "-").Trim('-');
        return T("machine." + slug + ".name");
    }

    private static string GetContentPackRuleDisplayName(LoadedContentPackRule loaded)
    {
        Match match = Regex.Match(loaded.Rule.DisplayName, @"^\{\{i18n:([A-Za-z0-9_.-]+)\}\}$");
        return match.Success
            ? loaded.Source.Translation.Get(match.Groups[1].Value).ToString()
            : loaded.Rule.DisplayName;
    }
