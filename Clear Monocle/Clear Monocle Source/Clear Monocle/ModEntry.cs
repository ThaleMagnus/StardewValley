...
public sealed class ModEntry : Mod {
    ...
    private void InitializeConfig() {
        ...
        ApplyPerformanceAuditConfigMigration();
    ...
    private static void ApplyPerformanceAuditConfigMigration() {
        const string migrationVersion = "7";
        if (!Versioning.IsOlderThan(Config.PerformanceAuditMigrationVersion, migrationVersion)) {
            return;
        }

        if (Config.Resample.EnabledLargeText) {
            Config.Resample.EnabledLargeText = false;
        }

        if (Config.Resample.EnabledSmallText) {
            Config.Resample.EnabledSmallText = false;
        }

        Config.SMAPI.ApplyGetDataPatch = false;
        Config.Extras.RenderTargetStackTraceDetection = false;
        Config.Extras.StableSort = false;
        Config.Extras.OpenGL.Enabled = false;
        Config.Extras.OpenGL.OptimizeTexture2DSetData = false;
        Config.Extras.OpenGL.OptimizeTexture2DGetData = false;

        if (Config.Debug.DisplayFrameTime) {
            Config.Debug.DisplayFrameTime = false;
        }

        if (Config.Debug.Logging.LogLevel < LogLevel.Warn) {
            Config.Debug.Logging.LogLevel = LogLevel.Warn;
        }

#if (!SHIPPING && !RELEASE) || LOG_MONITOR
        if (Config.Debug.Logging.SilenceOtherMods) {
            Config.Debug.Logging.SilenceOtherMods = false;
        }
#endif

        Config.PerformanceAuditMigrationVersion = migrationVersion;
        Config.ConfigVersion = Versioning.CurrentVersion.FileVersion!;
    }
    ...
    private const string UnderTestingMessage = "which is still in testing under Clear Monocle - results may vary";
    ...
    private void OnGameLaunched() {
        RadianceCompatibility.Apply(HarmonyInstance.Value, Helper.ModRegistry);
        ...
        _ = Task.Run(() => {
            try {
                ...
                Debug.Warning("Failed to check mod compatibility warnings", ex);
            }
        });
        _ = Task.Run(() => {
            try {
                ...
                Debug.Warning("Failed to initialize file cache", ex);
            }
        });

        Configuration.ConfigMenu.Setup.Initialize();
        ManagedSpriteInstance.ClearTimers();
    }

    private static void OnRendering(object? _, RenderingEventArgs _1) {
        DrawState.OnBeginDraw();
    }

    private static void OnRendered(object? _, RenderedEventArgs _1) {
        DrawState.OnPresent();
