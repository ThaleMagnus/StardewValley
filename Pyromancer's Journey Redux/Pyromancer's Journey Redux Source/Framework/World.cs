...
namespace ThaleMagnus.PyromancersJourney.Framework
{
    internal class World : IDisposable
    {
        ...
        public LevelWarp? Warp { get; private set; }
        ...
        public bool UsedInfiniteHealth { get; private set; }
        ...
        public World()
        {
            ...
            this.UsedInfiniteHealth = Mod.Instance.Config.InfiniteHealth;
            this.InitLevel("0");
        ...
        public void Update()
        {
            if (Mod.Instance.Config.InfiniteHealth)
                this.UsedInfiniteHealth = true;
        ...
        public void Render()
        {
            ...
            if (Mod.Instance.Config.ShowReticle)
            {
                int center = this.ScreenSize / 2;
                this.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null);
                this.SpriteBatch.Draw(
                    Game1.staminaRect,
                    new Rectangle(center, center, 1, 1),
                    ReticlePalette.Get(Mod.Instance.Config.ReticleColor)
                );
        ...
        [SuppressMessage("Reliability", "CA2000", Justification = "The resource is owned and disposed by the containing game object.")]
            ...
            for (int i = 1; i <= this.Map.Size.Y; ++i)
            {
                ...
                for (int ix = 0; ix < this.Map.Size.X; ++ix)
                {
                    ...
                    switch (lines[i][ix])
                    {
                        ...
                        default:
                            SpaceShared.Log.Warn("Unknown tile type " + lines[i][ix] + "!");
                            break;
                    }
                }
            }
