...
internal static class FileCache {
    ...
    private static readonly Lazy<TaskFactory> TaskFactory = new(
        () => new TaskFactory(new ThreadedTaskScheduler(threadNameFunction: index => $"FileCache IO Thread #{index}", threadPriority: ThreadPriority.BelowNormal))
    );
    ...
    private struct CacheHeader {
        ...
        internal Scaler Scaler = Scaler.xBRZ;
    ...
    internal static bool Save(
        ...
        try {
            ...
            TaskFactory.Value.StartNew(obj => {
                CurrentTaskLock.EnterReadLock();
                try {
                    ...
                    try {
                        ...
                        using (var writer = new BinaryWriter(new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None))) {
                            ...
                            new CacheHeader() {
                                ...
                                Scaler = scalerInfo?.Scaler ?? Scaler.xBRZ,
                                Gradient = gradient
                            }.Write(writer);
    ...
    static FileCache() {
        try {
            ...
            if (Config.FileCache.Enabled || Config.Debug.Sprite.DumpReference || Config.Debug.Sprite.DumpResample) {
