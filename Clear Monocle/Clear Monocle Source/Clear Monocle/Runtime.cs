...
internal static class Runtime {
    ...
    private static bool SupportsProcessorAffinity => OperatingSystem.IsWindows() || OperatingSystem.IsLinux();

    private static readonly bool EnableProcessAffinity = new Func<bool>(
        () => {
            if (!SupportsProcessorAffinity) {
                return false;
            }
    ...
    private static int AffinityCorrectionAttempted = 0;

    internal static void CorrectProcessorAffinity() {
        if (Interlocked.Exchange(ref AffinityCorrectionAttempted, 1) != 0) {
            return;
        }

        if (!EnableProcessAffinity || !SupportsProcessorAffinity) {
            return;
        }
