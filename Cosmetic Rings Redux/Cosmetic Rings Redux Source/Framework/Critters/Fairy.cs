...
namespace ThaleTheGreat.CosmeticRingsRedux.Framework.Critters
{
    internal sealed class Fairy : Critter
    {
        ...
        public Fairy(Vector2 position)
        {
            ...
            burstTimer = Game1.random.Next(1000, 8000);
            trailTimer = 250;
        }
        ...
        {
            light.radius.Value = 0.1f * (2.5f + (float)Math.Sin(2 * Math.PI * elapsedTime));
            elapsedTime = (elapsedTime + (float)time.ElapsedGameTime.TotalMilliseconds / 3000f) % 1f;

            burstTimer -= time.ElapsedGameTime.Milliseconds;
            if (burstTimer <= 0)
            {
                motionMultiplier = 5f;
                burstTimer = Game1.random.Next(1000, 8000);
            ...
            trailTimer -= time.ElapsedGameTime.Milliseconds;
            if (trailTimer <= 0 && motionMultiplier >= 1f)
            {
                trailTimer = 250;
                environment.temporarySprites.Add(new TemporaryAnimatedSprite(11, position, Color.White));
            }
            ...
            motionMultiplier -= 0.0005f * time.ElapsedGameTime.Milliseconds;
