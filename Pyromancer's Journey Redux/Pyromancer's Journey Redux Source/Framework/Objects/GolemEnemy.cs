...
namespace ThaleMagnus.PyromancersJourney.Framework.Objects
{
    internal class GolemEnemy : Enemy
    {
        ...
        public override RectangleF BoundingBox { get; } = new(-1.5f, -0.75f, 3f, 1.5f);

        private const int HealthMax = 50;
        ...
        {
            int width = 50;
            int x = this.World.ScreenSize - width - 4;
            int y = 4;
            int fill = (int)MathHelper.Clamp((width - 2) * (this.Health / (float)HealthMax), 0, width - 2);

            b.Begin();
            b.Draw(Game1.staminaRect, new Rectangle(x, y, width, 6), Color.Black);
            b.Draw(Game1.staminaRect, new Rectangle(x + 1, y + 1, width - 2, 4), Color.DarkRed);
            if (fill > 0)
                b.Draw(Game1.staminaRect, new Rectangle(x + 1, y + 1, fill, 4), Color.Red);
            b.End();
        }
