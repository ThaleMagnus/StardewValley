...
internal sealed class FashionSenseMirrorModule : WalletModule
{
    ...
    internal FashionSenseMirrorModule(ModEntry host)
        : base(host, ModuleKey, "module.fashion-sense.name", string.Empty, FashionSenseUniqueId)
    {
    }
    ...
    internal override void OnGameLaunched()
    {
        ...
        gmcm.AddPage(ModManifest, Host.GetModulePageId(ModuleKey), () => Host.Translate("module.fashion-sense.name"));
        gmcm.AddBoolOption(ModManifest, () => Config.Enabled, SetEnabled, () => Host.Translate("config.fashion-sense.enabled.name"), () => Host.Translate("config.fashion-sense.enabled.tooltip"), $"{ModuleKey}.Enabled");
        gmcm.AddBoolOption(ModManifest, () => Config.AutoStoreFromInventory, SetAutoStoreFromInventory, () => Host.Translate("config.fashion-sense.auto-store.name"), () => Host.Translate("config.fashion-sense.auto-store.tooltip"), $"{ModuleKey}.AutoStoreFromInventory");
        gmcm.AddKeybindList(ModManifest, () => Config.UseHandMirrorKey, value => Config.UseHandMirrorKey = value, () => Host.Translate("config.fashion-sense.use.name"), () => Host.Translate("config.fashion-sense.use.tooltip"), $"{ModuleKey}.UseHandMirrorKey");
    }
    ...
    private void ConvertInventoryMirrors(Farmer player)
    {
        ...
        InvalidatePowers();
        Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.hand-mirror-added"), HUDMessage.newQuest_type));
    }
    ...
    private bool ReturnMirrorToInventory(Farmer player, bool showMessage)
    {
        ...
        InvalidatePowers();
        if (showMessage)
            Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.hand-mirror-returned"), HUDMessage.newQuest_type));
        return true;
    }
