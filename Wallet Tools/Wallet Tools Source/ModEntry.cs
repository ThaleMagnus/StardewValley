...
public sealed class ModEntry : Mod
{
    ...
    private void AddPower(IDictionary<string, PowersData> powers, WalletToolKind kind)
    {
        ...
        {
            DisplayName = state.GetDisplayName(GetFallbackDisplayName(kind)),
    ...
    private WalletToolState CreateLockedPowerDisplayState(WalletToolKind kind)
    {
    ...
    internal string GetFallbackDisplayNameForApi(WalletToolKind kind)
    {
        return GetFallbackDisplayName(kind);
    }

    private string GetFallbackDisplayName(WalletToolKind kind)
    {
        return kind switch
        {
            WalletToolKind.Axe => T("tool.axe.name"),
            WalletToolKind.Pickaxe => T("tool.pickaxe.name"),
            WalletToolKind.Hoe => T("tool.hoe.name"),
            WalletToolKind.WateringCan => T("tool.watering-can.name"),
            WalletToolKind.Pan => T("tool.pan.name"),
            WalletToolKind.MilkPail => T("tool.milk-pail.name"),
            WalletToolKind.Shears => T("tool.shears.name"),
            _ => T("tool.generic.name")
        };
    }
    ...
    {
        GingerIslandExtraLocationsCompatibility.Apply(Harmony, Helper.ModRegistry, Monitor);
    ...
    private void RegisterIntegratedModulePageLinks()
    {
        ...
        GmcmApi.AddSectionTitle(ModManifest, () => T("config.integrated-wallets.section"));
        ...
        foreach (WalletModule module in IntegratedModules.Where(module => module.IsActive && module.HasConfigPage))
        {
            GmcmApi.AddPageLink(
                ...
                () => module.DisplayName,
                () => T("config.integrated-wallets.page-tooltip", new { moduleName = module.DisplayName })
            );
        }
    }
    ...
    internal string Translate(string key, object tokens)
    {
