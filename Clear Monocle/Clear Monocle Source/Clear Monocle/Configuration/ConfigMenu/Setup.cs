...
internal static class Setup {
    ...
    internal static void Initialize() {
        ...
        if (ModEntry.Self.Helper.ModRegistry.GetApi<IGenericModConfigMenuApi>("spacechase0.GenericModConfigMenu") is not { } configApi) {
        ...
        configApi.Register(
            mod: ModEntry.Self.ModManifest,
            reset: Reset,
            save: Save
        );

        configApi.OnFieldChanged(
            ModEntry.Self.ModManifest,
            OnValueChange
        );
        ...
        ProcessCategory(
            ...
            advanced: false,
            manifest: ModEntry.Self.ModManifest,
            config: configApi
        );
    ...
    internal static void ForceOpen() {
        ConfigApi?.OpenModMenu(ModEntry.Self.ModManifest);
    }
    ...
    private static T ExtractCombinedEnum<T>(Type enumType, string combinedName) where T : unmanaged {
        ...
        return (T)Convert.ChangeType(Enum.Parse(enumType, combinedName), typeof(T));
    }
    ...
    private static void OnMenuOpen() {
        ModEntry.Self.Helper.Events.Input.ButtonReleased += OnClick;
    ...
    private static void ProcessCategory(Serialize.Category? parent, Serialize.Category category, bool advanced, IManifest manifest, IGenericModConfigMenuApi config, bool first = false) {
        ...
        if (isRoot) {
            ...
            config.AddComplexOption(
                ...
                tooltip: () => Helper.Translation.Get("resampling.preview.tooltip"),
