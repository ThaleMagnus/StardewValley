...
internal sealed class TractorModModule : WalletModule
{
    ...
    private void RegisterGmcm()
    {
        ...
        try
        {
            ...
            AddBool(GmcmApi, nameof(Config.EnableTractorWalletSelector), () => Config.EnableTractorWalletSelector, value => Config.EnableTractorWalletSelector = value, "config.tractor.enabled.name", "config.tractor.enabled.tooltip");
            AddKeybind(GmcmApi, nameof(Config.CyclePreviousToolKey), () => Config.CyclePreviousToolKey, value => Config.CyclePreviousToolKey = value, "config.tractor.previous.name", "config.tractor.previous.tooltip");
            AddKeybind(GmcmApi, nameof(Config.CycleNextToolKey), () => Config.CycleNextToolKey, value => Config.CycleNextToolKey = value, "config.tractor.next.name", "config.tractor.next.tooltip");
            AddBool(GmcmApi, nameof(Config.ShowSelectorOverlay), () => Config.ShowSelectorOverlay, value => Config.ShowSelectorOverlay = value, "config.tractor.overlay.name", "config.tractor.overlay.tooltip");
            AddText(GmcmApi, nameof(Config.SelectorOverlayPosition), () => GetNormalizedOverlayPosition(), SetOverlayPosition, "config.tractor.position.name", "config.tractor.position.tooltip", OverlayPositionChoices);
            AddBool(GmcmApi, nameof(Config.PlayCycleSound), () => Config.PlayCycleSound, value => Config.PlayCycleSound = value, "config.tractor.cycle-sound.name", "config.tractor.cycle-sound.tooltip");
            AddBool(GmcmApi, nameof(Config.DebugLogging), () => Config.DebugLogging, value => Config.DebugLogging = value, "config.tractor.debug.name", "config.tractor.debug.tooltip");
    ...
    private void AddText(IGenericModConfigMenuApi gmcm, string fieldId, Func<string> getValue, Action<string> setValue, string nameKey, string tooltipKey, string[] allowedValues)
    {
        gmcm.AddTextOption(ModManifest, getValue, setValue, () => Host.Translate(nameKey), () => Host.Translate(tooltipKey), allowedValues, FormatOverlayPosition, fieldId);
    }

    private string FormatOverlayPosition(string value)
    {
        return value switch
        {
            OverlayTopOfScreen => Host.Translate("config.tractor.position.top"),
            OverlayLeftOfInventoryBar => Host.Translate("config.tractor.position.left"),
            OverlayRightOfInventoryBar => Host.Translate("config.tractor.position.right"),
            _ => value
        };
    }
