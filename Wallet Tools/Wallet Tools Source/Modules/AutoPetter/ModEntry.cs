...
internal sealed class WalletAutoPetterModule : WalletModule
{
    ...
    private void OnSaveLoaded(object? sender, SaveLoadedEventArgs e)
    {
        State = Context.IsMainPlayer
            ? Host.ReadSaveDataWithLegacy<SaveState>(StateKey, LegacyUniqueId) ?? new SaveState()
            : new SaveState();
    ...
    private void CollectAutoPetterFromInventory(bool showNotice)
    {
        ...
        if (showNotice && !SuppressNextStoreNotice)
        {
            ...
            string message = collected == 1
                ? Host.Translate("hud.auto-petter-stored", new { itemName = name })
                : Host.Translate("hud.auto-petters-stored", new { count = collected, itemName = name });
    ...
    private string GetStoredCountTooltipLine()
    {
        ...
        return count > 1 ? "\n" + Host.Translate("tooltip.auto-petter.stored-count", new { count }) : string.Empty;
    }
    ...
    private string GetAutoPetterDisplayName()
    {
        try
        {
        ...
        {
            return Host.Translate("item.auto-petter.name");
        }
    }

    private string GetAutoPetterDescription()
    {
        try
        {
        ...
        {
            return Host.Translate("item.auto-petter.description");
        }
    }
    ...
    private void RegisterGmcm()
    {
        ...
        gmcm.AddSectionTitle(ModManifest, () => Host.Translate("config.auto-petter.section"));
        gmcm.AddBoolOption(ModManifest, () => Config.Enabled, SetEnabled, () => Host.Translate("config.auto-petter.enabled.name"), () => Host.Translate("config.auto-petter.enabled.tooltip"));
        gmcm.AddBoolOption(ModManifest, () => Config.AutoStoreFromInventory, value => Config.AutoStoreFromInventory = value, () => Host.Translate("config.auto-petter.auto-store.name"), () => Host.Translate("config.auto-petter.auto-store.tooltip"));
        gmcm.AddBoolOption(ModManifest, () => Config.ShowWalletIcon, value => Config.ShowWalletIcon = value, () => Host.Translate("config.auto-petter.icon.name"), () => Host.Translate("config.auto-petter.icon.tooltip"));
        gmcm.AddBoolOption(ModManifest, () => Config.ShowStoredMessage, value => Config.ShowStoredMessage = value, () => Host.Translate("config.auto-petter.message.name"), () => Host.Translate("config.auto-petter.message.tooltip"));

        gmcm.AddSectionTitle(ModManifest, () => Host.Translate("config.auto-petter.effect.section"));
        gmcm.AddBoolOption(ModManifest, () => Config.ApplyFriendshipGain, value => Config.ApplyFriendshipGain = value, () => Host.Translate("config.auto-petter.friendship.name"), () => Host.Translate("config.auto-petter.friendship.tooltip"));
        gmcm.AddNumberOption(ModManifest, () => Config.FriendshipPointsPerDay, value => Config.FriendshipPointsPerDay = value, () => Host.Translate("config.auto-petter.points.name"), () => Host.Translate("config.auto-petter.points.tooltip"), min: 0, max: 15, interval: 1, formatValue: value => value.ToString());
