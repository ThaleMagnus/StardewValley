...
internal sealed class ManagedSpriteInstance : IByteSize, IDisposable {
    ...
    internal static bool Validate(XTexture2D texture, bool clean = false) {
        ...
        bool InnerValidate() {
            ...
            bool IsLargeFont() {
                var textureName = texture.NormalizedName();
                if (
                    textureName.StartsWith(@"Fonts\", StringComparison.OrdinalIgnoreCase) &&
                    !textureName.Contains("Small", StringComparison.OrdinalIgnoreCase) &&
                    !textureName.Contains("tiny", StringComparison.OrdinalIgnoreCase)
                ) {
                    return true;
                }
    ...
    internal static ManagedSpriteInstance? FetchOrCreate(XTexture2D texture, Bounds source, uint expectedScale, bool sliced, out bool allowCache) {
        ...
        string GetMetadataString() {
            return isCached.HasValue ?
                $" ({(useAsync ? "async" : "sync")} {(isCached.Value ? "cached" : "uncached")})" :
                $" ({(useAsync ? "async" : "sync")})";
        }
        ...
        if (spriteInfoInitializer.ReferenceData is null) {
            Debug.Trace($"Texture Data fetch for {GetNameString()} was {"blocked"}; retrying later{GetRemainingTime()}");
            allowCache = false;
            return currentInstance;
        }
    ...
    private Func<Delegate?>? SuspendChain(bool clearChildrenIfDispose = false) {
        ...
        Debug.Trace($"Sprite Instance '{Name}' {"Suspended"}");
    ...
    internal bool Resurrect(XTexture2D texture, ulong spriteMapHash) {
        ...
        Debug.Trace($"Sprite Instance '{Name}' {"Resurrected"}");
