...
internal sealed class HorseFluteModule : WalletModule
{
    ...
    internal HorseFluteModule(ModEntry host)
        : base(host, ModuleKey, "module.horse-flute.name", string.Empty)
    {
    }
    ...
    internal override void OnGameLaunched()
    {
        ...
        if (gmcm is not null)
        {
            ...
            gmcm.AddPage(ModManifest, Host.GetModulePageId(ModuleKey), () => Host.Translate("module.horse-flute.name"));
            gmcm.AddBoolOption(ModManifest, () => Config.Enabled, value => SetEnabled(value), () => Host.Translate("config.horse-flute.enabled.name"), () => Host.Translate("config.horse-flute.enabled.tooltip"), $"{ModuleKey}.Enabled");
            gmcm.AddBoolOption(ModManifest, () => Config.AutoStoreFromInventory, SetAutoStoreFromInventory, () => Host.Translate("config.horse-flute.auto-store.name"), () => Host.Translate("config.horse-flute.auto-store.tooltip"), $"{ModuleKey}.AutoStoreFromInventory");
            gmcm.AddKeybindList(ModManifest, () => Config.UseHorseFluteKey, value => Config.UseHorseFluteKey = value, () => Host.Translate("config.horse-flute.use.name"), () => Host.Translate("config.horse-flute.use.tooltip"), $"{ModuleKey}.UseHorseFluteKey");
        }
    }
    ...
    private void ConvertInventoryFlutes(Farmer player)
    {
        ...
        InvalidatePowers();
        Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.horse-flute-added"), HUDMessage.newQuest_type));
    }
    ...
    private bool ReturnFluteToInventory(Farmer player, bool showMessage)
    {
        ...
        InvalidatePowers();
        if (showMessage)
            Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.horse-flute-returned"), HUDMessage.newQuest_type));
        return true;
    }
