...
internal static class Harmonize {
    ...
    private static void ApplyPatch(Harmony @this, Type type, MethodInfo? method, HarmonizeAttribute attribute) {
        try {
            ...
            if (attribute.ForMod is not null && ModEntry.Self.Helper.ModRegistry.Get(attribute.ForMod) is null) {
    ...
    private static void ApplyPatches(Harmony @this, Type type, MethodInfo method, IEnumerable<HarmonizeAttribute> attributes, bool early) {
        try {
            foreach (var attribute in attributes.OrderBy(attribute => attribute.GetType().FullName, StringComparer.Ordinal)) {
    ...
    private static readonly HashSet<Type> MinimalPatchTypes = new() {
        typeof(Patches.PSpriteBatch.Begin),
        typeof(Patches.PSpriteBatch.Patch.Draw),
        typeof(Patches.PSpriteBatch.Patch.DrawString),
        typeof(Patches.PTexture2D)
    };
    ...
    internal static void ApplyPatches(this Harmony @this, bool early) {
        ...
        foreach (var type in assembly.GetTypes().OrderBy(type => type.FullName, StringComparer.Ordinal)) {
            if (!MinimalPatchTypes.Contains(type)) {
                continue;
            }
            var typeAttributes = type.GetCustomAttributes<HarmonizeFinalizeCatcherFixedAttribute>()
                .OrderBy(attribute => attribute.GetType().FullName, StringComparer.Ordinal);
            ...
            foreach (var method in type.GetMethods(StaticFlags).OrderBy(method => method.MetadataToken)) {
    ...
    private static MethodBase? GetPatchMethod(Type type, string name, MethodInfo method, HarmonizeAttribute attribute, bool instance, bool isFinalizer, bool transpile = false) {
        ...
        if (typeMethod is null) {
            Debug.Warning($"Failed to patch {type.Name}.{name}");
            return null;
        }
        return typeMethod;
    }
