...
internal sealed class CoinCollectorReduxModule : WalletModule
{
    ...
    private void OnAssetRequested(object? sender, AssetRequestedEventArgs e)
    {
        ...
        e.Edit(asset =>
        {
            ...
            {
                DisplayName = state.GetDisplayName(Monitor, Host.Translate("item.metal-detector.name")),
                Description = state.GetDescription(Monitor, Host.Translate("item.metal-detector.description")),
    ...
    private void RegisterGmcm()
    {
        ...
        try
        {
            ...
            AddBool(gmcm, nameof(Config.ModEnabled), () => Config.ModEnabled, SetModEnabled, "config.coin-collector.enabled.name", "config.coin-collector.enabled.tooltip");
            AddBool(gmcm, nameof(Config.EnablePassiveWalletDetection), () => Config.EnablePassiveWalletDetection, value => Config.EnablePassiveWalletDetection = value, "config.coin-collector.passive.name", "config.coin-collector.passive.tooltip");
            AddBool(gmcm, nameof(Config.EnableManualUseHotkey), () => Config.EnableManualUseHotkey, value => Config.EnableManualUseHotkey = value, "config.coin-collector.manual.name", "config.coin-collector.manual.tooltip");
            AddBool(gmcm, nameof(Config.PlayToolSwapSound), () => Config.PlayToolSwapSound, value => Config.PlayToolSwapSound = value, "config.coin-collector.swap-sound.name", "config.coin-collector.swap-sound.tooltip");
            AddBool(gmcm, nameof(Config.ShowHudMessageWhenStored), () => Config.ShowHudMessageWhenStored, value => Config.ShowHudMessageWhenStored = value, "config.coin-collector.stored-message.name", "config.coin-collector.stored-message.tooltip");
            AddKeybind(gmcm, nameof(Config.UseMetalDetectorHotkey), () => Config.UseMetalDetectorHotkey, value => Config.UseMetalDetectorHotkey = value, "config.coin-collector.use-detector.name", "config.coin-collector.use-detector.tooltip");
            GmcmRegistered = true;
        }
        catch (Exception ex)
        {
        ...
        try
        {
            for (int i = 0; i < player.Items.Count; i++)
            {
                ...
                if (Config.ShowHudMessageWhenStored)
                    Game1.addHUDMessage(new HUDMessage(Host.Translate("hud.metal-detector-stored"), HUDMessage.newQuest_type));
    ...
    private sealed class StoredDetectorState
    {
        ...
        public string GetDisplayName(IMonitor monitor, string fallbackDisplayName)
        {
            Item? detector = CreateItem(monitor);
            return detector is not null && !string.IsNullOrWhiteSpace(detector.DisplayName) ? detector.DisplayName : fallbackDisplayName;
        }

        public string GetDescription(IMonitor monitor, string fallbackDescription)
        {
            Item? detector = CreateItem(monitor);
            return detector?.getDescription() ?? fallbackDescription;
        }
